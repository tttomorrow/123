...
sql="SELECT c.relname FROM pg_catalog.pg_class c LEFT JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace WHERE c.relkind IN ('p') AND n.nspname <> 'pg_catalog' AND n.nspname <> 'information_schema' AND n.nspname !~ '^pg_toast' AND pg_catalog.pg_table_is_visible(c.oid) and n.nspname='${SCHEMA}';"
...
handle_partition(){
    ...
    for(( j=2;j<${#p_array[@]}-2;j++)) do
        ...
        sed -i 24a\ "sed -i \"/^COPY[[:space:]]$SCHEMA.${p_array[j]}[[:space:]](.*)[[:space:]]FROM[[:space:]]stdin;/ { s/${p_array[j]}/$1/g;}\"  $PG_DATA_DIR/pg.sql" ~/pg2og_migration/export.sh
    done;
...
if [ ! -f "~/pg2og_migration/partition_table_information.txt" ]; then
    touch ~/pg2og_migration/partition_table_information.txt
fi
